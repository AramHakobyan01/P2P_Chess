#include "Queens.h"
void Queens::MoveQueen(Coordinates coordinates, Coordinates new_coordinates) {
	if (halper.CanQueenMove(coordinates, new_coordinates)) {
		if (figur[coordinates.x][coordinates.y].color == game_color) {
			if (figur[new_coordinates.x][new_coordinates.y].name == None) {
				figur[new_coordinates.x][new_coordinates.y] = { game_color,Queen};
				figur[coordinates.x][coordinates.y] = { NULL,None };
				if (halper.AreChacking(game_color)) {
					figur[coordinates.x][coordinates.y] = { game_color,Queen };
